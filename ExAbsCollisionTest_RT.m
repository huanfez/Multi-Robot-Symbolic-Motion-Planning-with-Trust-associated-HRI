...
timeStart = tic;
MakeMovie = 0;
MovieName = 'ExAbsCollisionTest_RT.avi';
%% Environment
...
%                 if (mTrack{m}(2) - mTrack{m}(1)) == -(mTrack{m2}(2) - mTrack{m2}(1))
                if mTrack{m}(2) == mTrack{m2}(1) && mTrack{m2}(2) == mTrack{m}(1)
...
%             sensedObstacles = [sensedObstacles, cPath{m}(pointCounter(m))];
            ...
            while isempty(cPath{m})
                ...
                % instance only
                makeSMV_v2(fileName, gridWidth, gridLength, gridChunk(m,:), newStart(m), [sensedObstacles mTrack{m2}], ltlspec{m});
                [~,output] = system(['cd ' pathNuSMV ' & NuSMV ' filePath '\' fileName]);
                [xPath{m},yPath{m},cPath{m}] = getPath(output, gridWidth, gridLength);
            end
