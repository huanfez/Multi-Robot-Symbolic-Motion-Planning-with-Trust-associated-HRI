...
MakeMovie = 0;
MovieName = 'C:\Users\yue6\Dropbox\David_Adam_Spencer\Thesis\Codes\Chapter4\HRI_video_v3.avi';
% frameRate =  1/.02;
frameRate = 20;
autoPlay = 1;
...
%     [0 100 310 452]
    if ismember(k,[1 100 310 452])
        for mTemp = 1:M
            hTemp(mTemp) = plot(x{mTemp}(1,1:k),x{mTemp}(2,1:k),'k-','LineWidth',3);
        end
        pause
%         saveas(1,['Figures\Plan' num2str(subCounter) '.fig'],'fig')
%         saveas(2,['Figures\Path' num2str(subCounter) '.fig'],'fig')
...
figure(5)
plot(mTime{1},mTrust(1,1:kFin(1))+1.5,mTime{2},mTrust(2,1:kFin(2))+1.5,mTime{3},mTrust(3,1:kFin(3))+1.5,'LineWidth',3)
legend('Robot 1','Robot 2','Robot 3','Location','southeast')
xlabel('Time step'),ylabel('Trust')
set(gca,'fontsize',14)
axis([0 max(kFin) 0 10.5+1.5])
