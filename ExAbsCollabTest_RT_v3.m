...
MakeMovie = 1;
MovieName = 'D:\Users\daspenc\Videos\LHTRLI\ColAvVid_v4Ext.avi';
frameRate =  24;%1/.02;%
autoPlay = 1;
...
if numOFigs == 2
    figure('units','pixels','outerposition',[0 0 1400+(1400-1384) 933+(933-840)]); % if wanted on one figure
else
...
%         if hOcc == 0 && rewardCount(m) > 0 && rewardCount(m) == max(rewardCount)
        ...
        if m == hOcc
            if dt*(k-hReacStart) >= hReacTime % once human has had enough time to plan
                HumanControlScript_v3 % run human plan version of code
        ...
        % if flagged to replan
            fprintf('colReplan')
            CollisionReplanScript_v3ColAv % Replan (seperated for sanity)
        end
...
%             rewardCount(m) = rewardCount(m) - 1;
            ObstacleReplanScriptColAv % Replan (seperated for sanity)
        end
    end % end agent loop
...
%     pause(1/30)
    ...
    if MakeMovie == 1 && k > 349 && k < 676
        frame = getframe(gcf);
        writeVideo(movieObj,frame);
    end
    
    if MakeMovie == 1 && k == 676
        close(movieObj)
    end
