...
while isempty(cPath)
    ...
    gridChunk = getChunk(start, goals, gridWidth, gridLength, expansion);
...
for k = 1:N-1
    ...
    if ismember(cPath(pointCounter),obstacles)
        ...
        while isempty(cPath)
            ...
            gridChunk = getChunk(newStart, goalsLeft, gridWidth, gridLength, expansion);
        ...
        % makeSMV(fileName, gridWidth, gridLength, newStart, sensedObstacles, ltlspec);
        % [~,output] = system(['cd ' pathNuSMV ' & NuSMV ' filePath '\' fileName]);
        % [xPath,yPath,cPath] = getPath(output, gridWidth, gridLength);
