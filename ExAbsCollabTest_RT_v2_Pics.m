...
% Collision == 416, HumanReplan1 == 97
    if k == 97 || k == 416
        ...
        for mSub = 1:M
            ...
            if k == 97 && mSub == 2
                hArrow{mSub} = arrow([xPath{mSub}(pointCounter(mSub)-1:cEnd-1)',yPath{mSub}(pointCounter(mSub)-1:cEnd-1)'],...
                    [xPath{mSub}(pointCounter(mSub):cEnd)',yPath{mSub}(pointCounter(mSub):cEnd)'],10);
            end
            if k == 416
                hArrow{mSub} = arrow([xPath{mSub}(pointCounter(mSub)-1)',yPath{mSub}(pointCounter(mSub)-1)'],...
                    [xPath{mSub}(pointCounter(mSub))',yPath{mSub}(pointCounter(mSub))'],10);
            end
...
%         if hOcc == 0 && rewardCount(m) > 0 && rewardCount(m) == max(rewardCount)
        ...
        if m == hOcc
            if dt*(k-hReacStart) >= hReacTime % once human has had enough time to plan
                HumanControlScript % run human plan version of code
